...
export class AnalyticsService {
  ...
  async trackArticleView(articleId: string, dto: ArticleViewDto, user?: AuthenticatedUser) {
    ...
    return this.sequelize.transaction(async (transaction) => {
      const [row] = await this.sequelize.query(
        ...
        { replacements: { articleId, userId: user?.id ?? null, visitorHash: dto.visitorHash ?? null, userAgent: dto.userAgent ?? null, ipAddress: dto.ipAddress ?? null }, type: QueryTypes.SELECT, transaction }
      );
      await this.sequelize.query(
        ...
        { replacements: { articleId, payload: JSON.stringify({ articleId, userId: user?.id ?? null }) }, type: QueryTypes.INSERT, transaction }
      );
      return row;
    });
  }
  ...
  async articleSummaryAdmin() {
    return this.sequelize.query(
      `
      SELECT a.id AS "articleId", a.title AS "articleTitle", count(av.id)::int AS views
      FROM articles a
  ...
  private async hasActiveSubscription(userId?: string, transaction?: Transaction) {
