...
export class CategoriesService {
    ...
    try {
      const category = await this.sequelize.transaction(async (transaction) => {
        const [row] = await this.sequelize.query<{ id: string }>(
          ...
          RETURNING id;
          `,
          { replacements: { name: dto.name, slug: dto.slug, description: dto.description ?? null, isActive: dto.isActive }, type: QueryTypes.SELECT, transaction }
        );
        await this.audit(actorUserId, row.id, 'category.created', dto, transaction);
        return this.get(row.id, transaction);
      });
      await this.invalidateReadCaches();
      return category;
    } catch (error) {
      if (error instanceof Error && /categories_slug|unique|duplicate key/i.test(error.message)) {
  ...
  async update(id: string, dto: UpdateCategoryDto, actorUserId: string) {
    ...
    try {
      const category = await this.sequelize.transaction(async (transaction) => {
        ...
        await this.audit(actorUserId, id, 'category.updated', dto, transaction);
  ...
  async get(id: string, transaction?: Transaction) {
  ...
  private async invalidateReadCaches() {
